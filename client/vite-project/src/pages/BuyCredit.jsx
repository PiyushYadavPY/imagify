...
const BuyCredit = () => {
  ...
  const initPay = async (order) => {
    const options = {
      ...
      description: "Imagify Credits Purchase",
      order_id: order.id,
      ...
      handler: async (response) => {
        ...
        try {
          const { data } = await axios.post(
            backendUrl + "/api/user/verify-razor-pay",
            response,
            { headers: { token } }
          );
          if (data.success) {
  ...
  const paymentRazorpay = async (planId) => {
    ...
    try {
      ...
      console.log("sending data:", { planId });
      const { data } = await axios.post(
  ...
  return (
    ...
    >
      <button className="border border-gry-400 px-10 py-2 rounded-full mb-6">
        Our Plans
      </button>
